...
bool GossipHello_npc_arena_honor(Player* pPlayer, Creature* pCreature)
{
	...
	if(TScriptConfig.GetBoolDefault("NPCArenaHonor.OnlyGMs", false)) // If NPCArenaHonor.OnlyGMs is enabled in scriptdev2.conf
	...
	bool EnableTradeArenatoHonor = TScriptConfig.GetBoolDefault("NPCArenaHonor.EnableTradeArenatoHonor", true);
	bool EnableTradeHonortoArena = TScriptConfig.GetBoolDefault("NPCArenaHonor.EnableTradeHonortoArena", true);
...
case 1000: // Change xx Arena to xx Honor
        if (pPlayer->GetArenaPoints() >= (TScriptConfig.GetFloatDefault("ArenaPoints",0)))
        {
            pPlayer->ModifyArenaPoints(-(TScriptConfig.GetFloatDefault("ArenaPoints",0))); 
            pPlayer->ModifyHonorPoints(+(TScriptConfig.GetFloatDefault("HonorPoints",0)));
        }else {
...
case 2000: // Change xx Honor to xx Arena
        if (pPlayer->GetHonorPoints() >= (TScriptConfig.GetFloatDefault("HonorPoints",0)))
        {
            pPlayer->ModifyHonorPoints(-(TScriptConfig.GetFloatDefault("HonorPoints",0))); 
            pPlayer->ModifyArenaPoints(+(TScriptConfig.GetFloatDefault("ArenaPoints",0)));
        }else {
