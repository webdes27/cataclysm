...
void Aura::HandleModStealth(bool apply, bool Real)
{
    if (apply)
    {
        ...
        if (Real)
        {
            ...
            if(m_target->GetAurasByType(SPELL_AURA_MOD_STEALTH).size()<=2)
            {
...
{
/*    if(m_target->GetTypeId() != TYPEID_PLAYER)		// hex for creatures
        return;*/
